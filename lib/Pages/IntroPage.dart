...
import 'package:symphonia_mobile_app/widgets/ui_widgets/info_widget.dart';
import '../consts.dart';
    ...
    return Scaffold(
      ...
      body: ListView(
        children: <Widget>[
          ...
          Transform.translate(offset: Offset(0, 0), child: secondPart(context)),
        ],
      ),
    );
  }
    ...
    child: Column(
      children: <Widget>[
        ...
        Padding(
          ...
          child: drawPurpleReg(context),
        ),
        // for كلمة الأفتتاحية للتطبيق ....
        Padding(
          padding: EdgeInsets.only(top: 30.0),
          child: dynamicIntroStatm(context),
        ),
...
// تابع لرد النص التعريفي عم التطبيق مع معالجة تنصيقات وحجم الخط على حسب الجهاز العارض
Widget dynamicIntroStatm(BuildContext context) {
  double font_size;
  return InfoWidget(
    builder: (context, deviceInfo) {
      ...
      if (deviceInfo.deviceType == DeviceScreenType.Mobile) {
        if (deviceInfo.orientation == Orientation.landscape) {
          font_size = 23;
        } else {
          font_size = 18;
...
Widget drawPurpleReg(BuildContext context) {
  return InfoWidget(builder: (context, deviceInfo) {
    // print('orientationInfo : ${deviceInfo.orientation}');
    // print('deviceTypeInfo : ${deviceInfo.deviceType}');
    // print('screenHeightInfo : ${deviceInfo.screenHeight}');
    // print('screenWidthInfo : ${deviceInfo.screenWidth}');
    // print('localHeightInfo : ${deviceInfo.localHeight}');
    // print('localWidthInfo : ${deviceInfo.localWidth}');
    return Column(
      children: <Widget>[
        Center(
          child: Container(
            width: deviceInfo.screenWidth / 2.3,
            height: deviceInfo.screenWidth / 3.5,
            decoration: BoxDecoration(
            ...
            child: Center(
              child: SvgPicture.string(
                ...
                width: deviceInfo.localWidth / 1.5,
                height: deviceInfo.localHeight / 1.5,
              ),
            ),
          ),
        ),
  ...
  return InfoWidget(
    builder: (context, deviceInfo) {
      return GestureDetector(
        ...
        child: Container(
          width: deviceInfo.screenWidth / 1.2,
          height: (deviceInfo.orientation == Orientation.landscape) &&
                  (deviceInfo.deviceType == DeviceScreenType.Mobile)
